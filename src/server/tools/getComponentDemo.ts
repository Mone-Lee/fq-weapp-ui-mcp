...
export const getComponentDemoSchema = {
  componentName: z.string().describe('组件名称 (如 FQButton)'),
  token: z.string().optional().describe('可选 GitLab Token')
};

export const getComponentDemoHandler = async ({ componentName, token }: { componentName: string, token?: string | undefined }): Promise<{ content: { type: 'text'; text: string }[] }> => {
